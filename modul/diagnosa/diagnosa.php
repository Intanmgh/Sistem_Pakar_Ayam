...
<?php
$act = isset($_GET['act']) ? $_GET['act'] : '';  // Memastikan 'act' ada sebelum mengaksesnya

switch ($act) {
  default:
    if (isset($_POST['submit'])) {  // Memastikan 'submit' ada sebelum memproses
        ...
        while ($rgejala = mysqli_fetch_array($sqlgejala)) {
          ...
          for ($i = 0; $i < count($_POST['kondisi']); $i++) {
            ...
            if ($rgejala['kode_gejala'] == $gejala) {
              ...
              if ($cf * $cflama < 0) {
                $cflama = ($cflama + $cf) / (1 - min(abs($cflama), abs($cf)));  // Memperbaiki penggunaan min
              }
              if (($cf < 0) && ($cf * $cflama >= 0)) {
                $cflama = $cflama + ($cf * (1 + $cflama));
              }
